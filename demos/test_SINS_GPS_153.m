...
kfFD.fd.delaySteps = 10;         % use state that is 10 updates old for soft-fault detection
kfFD.fd.softWindow = 10;         % sliding window length for delayed NIS statistic
kfFD.fd.softThreshold = 5.5;     % threshold on mean delayed NIS (tuned for 3-D GPS position)
kfFD.fd.softComponentThreshold = 9; % per-component |z|>3 gate on delayed residuals
...
[avpAll, avpFD, xkpkAll, xkpkFD, nisLog, faultFlagLog, detectLog, dwellLog, ...
 delayedNisLog, slidingStatLog, softTripLog, softCompLog] = ...
    prealloc(rows, 10, 10, 2*kfAll.n+1, 2*kfFD.n+1, 1, 1, 1, 1, 1, 1, 1, 1);
...
for k = 1:nn:len-nn+1
    ...
    if mod(t,1)==0
        ...
        delayedNisLog(ki)   = kfFD.fd.delayedNIS;
        slidingStatLog(ki)  = kfFD.fd.slidingStatistic;
        faultFlagLog(ki)    = faultActive;
        detectLog(ki)       = any(kfFD.fd.isOutlier);
        dwellLog(ki)        = any(kfFD.fd.inDwell);
        softTripLog(ki)     = kfFD.fd.softExceeded;
        softCompLog(ki)     = any(kfFD.fd.softComponentExceeded);
        ki = ki + 1;
    end
    timebar;
end
...
nisLog(ki:end) = []; delayedNisLog(ki:end) = [];
...
myfigure('Sliding residual test (delayed estimate)');
softThreshold = kfFD.fd.softThreshold;
subplot(2,1,1);
plot(tMeas, delayedNisLog, 'LineWidth', 1.2); hold on;
plot(tMeas, slidingStatLog, 'r', 'LineWidth', 1.2);
yline(softThreshold, 'k--', 'LineWidth', 1.1);
grid on;
legend('Instant delayed NIS','Sliding mean','Threshold','Location','best');
title('Delayed-residual energy for soft-fault detection');
xlabel('Time / s'); ylabel('NIS');
subplot(2,1,2);
stairs(tMeas, softTripLog, 'm', 'LineWidth', 1.2); hold on;
stairs(tMeas, softCompLog, 'c--', 'LineWidth', 1.1);
stairs(tMeas, faultFlagLog, 'k:', 'LineWidth', 1.1);
grid on; xlabel('Time / s'); ylabel('Flag');
ylim([-0.1 1.1]);
legend('Sliding gate trip','Component gate trip','Injected fault','Location','best');
