...
%                 subplot(122), hold on, plot(t, err(:,1:2)/glv.min, strk, 'LineWidth',2); xygo('mu'); mylegend('mux','muy');
        ...
        case {'a', 'y'},
            for k=1:kk
                ...
                for idx = 1:3
                    plotErrorComponent(ptype, attLabels{idx}, t, err(:,idx)/glv.min, strk, 'Attitude Error', k, kk);
         ...
         case 'av',
            for k=1:kk
                ...
                for idx = 1:3
                    plotErrorComponent(ptype, velLabels{idx}, t, err(:,idx+3), strk, 'Velocity Error', k, kk);
        ...
        case 'avp',
            for k=1:kk
                ...
                for idx = 1:3
                    posGroup = struct('figureTag', ['avpcmpplot_', ptype, '_position'], ...
                        'figureName', 'Position Error Components', 'numRows', 3, 'numCols', 1, ...
                        'subplotIndex', idx);
                    plotErrorComponent(ptype, posLabels{idx}, t, posData{idx}, strk, 'Position Error', k, kk, posGroup);
...
%                 subplot(121), hold on, plot(t, [avp(:,4:6),sqrt(avp(:,4).^2+avp(:,5).^2+avp(:,6).^2)], strk, 'LineWidth',2); xygo('V');
        ...
        case 've',
            for k=1:kk
                ...
                err = avpcmp(avp, avp0, 'noatt'); t = err(:,end);
                plotErrorComponent(ptype, 'dVE', t, err(:,4), strk, 'Velocity Error', k, kk);
        ...
        case 'vn',
            for k=1:kk
                ...
                err = avpcmp(avp, avp0, 'noatt'); t = err(:,end);
                plotErrorComponent(ptype, 'dVN', t, err(:,5), strk, 'Velocity Error', k, kk);
        ...
        case 'vu',
            for k=1:kk
                ...
                err = avpcmp(avp, avp0, 'noatt'); t = err(:,end);
                plotErrorComponent(ptype, 'dVU', t, err(:,6), strk, 'Velocity Error', k, kk);
        ...
        case 'vb',
            for k=1:kk
                ...
                for idx = 1:3
                    plotErrorComponent(ptype, vbLabels{idx}, t, err(:,idx), strk, 'Body Velocity Error', k, kk);
