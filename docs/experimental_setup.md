...
传感器噪声标准差定义在 `config.m:17-23`，参照 RTCA DO-236E 导航级 IRS 性能指标：

**陀螺仪偏置不稳定性**：

| 传感器 | 原始值 | 单位转换 | σ (rad/s) |
|--------|--------|---------|-----------|
| INS1 / INS2 | 0.005 deg/h | × π/(180×3600) | 2.424×10⁻⁷ |
| ISIS | 0.05 deg/h | × π/(180×3600) | 2.424×10⁻⁶ |

**加速度计偏置不稳定性**：

| 传感器 | 原始值 | 单位转换 | σ (m/s²) |
|--------|--------|---------|----------|
| INS1 / INS2 | 5×10⁻⁵ g | × 9.8 | 4.9×10⁻⁴ |
| ISIS | 1×10⁻³ g | × 9.8 | 9.8×10⁻³ |

> ISIS 的陀螺噪声是 INS 的 **10 倍**，加速度计噪声是 INS 的 **20 倍**。
...
|------|-----------|---------|---------|---------|-------------|---------|
| 1 | INS2 Z轴陀螺（航向通道） | 6 | gyro | 硬故障（阶跃） | [120, 135]、[440, 455]、[595, 610] | 0.8、1.5、3.0 deg/h |
| 2 | INS1 Y轴加速度计（横向通道） | 2 | acc | 硬故障（阶跃） | [100, 115]、[420, 435]、[575, 590] | 0.008g、0.015g、0.03g |
| 3 | INS1 X轴陀螺（水平通道） | 1 | gyro | 软故障（斜坡） | [260, 310] | 0.015 deg/h/s |
...
**条件 1 示例**（INS2 Z轴陀螺，航向通道）：
- 第 1 段：t ∈ [120, 135]s，叠加 0.8 deg/h 偏差
- 第 2 段：t ∈ [440, 455]s，叠加 1.5 deg/h 偏差
- 第 3 段：t ∈ [595, 610]s，叠加 3.0 deg/h 偏差

**条件 2 示例**（INS1 Y轴加速度计，横向通道）：
- 第 1 段：t ∈ [100, 115]s，叠加 0.008g 偏差
- 第 2 段：t ∈ [420, 435]s，叠加 0.015g 偏差
- 第 3 段：t ∈ [575, 590]s，叠加 0.03g 偏差
...
**条件 3 示例**（INS1 X轴陀螺，水平通道）：
- 时间窗口：t ∈ [260, 310]s（持续 50 秒）
- 斜率：0.015 deg/h/s
- 参考时间：t_ref = 260s
- 故障增长：从 0 线性增长到 0.015 × 50 = 0.75 deg/h
