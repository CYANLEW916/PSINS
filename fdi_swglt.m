...
%                 N_dwell, rho_threshold, P_isol, min_isolation_votes
%
%   Output:
...
% See also  compute_parity_matrix, compute_adaptive_threshold, fdi_wglt.
    ...
    if isfield(cfg, 'min_isolation_votes')
        min_isolation_votes = cfg.min_isolation_votes;
    else
        min_isolation_votes = 3;
    end
    ...
    for k = 1:N
        ...
        n_votes = margin_ok + posterior_ok + dwell_ok;

        if (n_votes >= min_isolation_votes) && dwell_ok
            isolated(k) = k_star;
            iso_status(k) = 2;
            ci_star = c_tilde(:, k_star);
            f_hat(k) = sigma_vec(k_star) * (ci_star' * P_bar) / c_norm_sq(k_star);
        elseif n_votes >= min_isolation_votes
            isolated(k) = k_star;
            iso_status(k) = 1;
            ci_star = c_tilde(:, k_star);
            f_hat(k) = sigma_vec(k_star) * (ci_star' * P_bar) / c_norm_sq(k_star);
